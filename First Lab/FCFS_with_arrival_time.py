...
process_list = []       
for i in range(3):
    process = ProcessTowhid(i, f"Process{i}", random.randint(0,2), random.randint(3,6))
...
for i in range(total_clock):
    arrived_processes = [(process.process_id,process.arrival_time) for process in (process_list) if process.arrival_time <= i and process.remaining_burst > 0]
    #arrived_id = [process[1] for process in arrived_processes]
    print(arrived_processes) 
    running = sorted(arrived_processes, key=lambda x: (x[1], x[0]))[0][0] if arrived_processes != [] else None
